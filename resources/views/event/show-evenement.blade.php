...
@pushonce('styles')
    @vite([
        ...
        'resources/css/evenements/show-evenement.scss',
                ...
                @if ($eventIsVisible && $canSeeEvent)
                    ...
                    <div class="header">
                        ...
                        @if ($evenement->confidentiel != 0)
                            <p id="confidentiel" title="Cet évènement n'est visible que pour votre campus. Ne pas partager"
                                class="tooltip"><i class="fa-solid fa-lock" id="confidentiel-icon"></i>Cet évènement est
                                confidentiel</p>
                        @endif
                        @if (strftime('%A %d %B', strtotime($evenement->temps_debut)) == strftime('%A %d %B', strtotime($evenement->temps_fin)))
                            {{-- Si début et fin le même jour --}}
                            <p><i
                                    class="fa-regular fa-calendar"></i>{{ ucwords(utf8_encode(strftime('%A %d %B de %H:%M', strtotime($evenement->temps_debut)))) . ' à ' . utf8_encode(strftime('%H:%M', strtotime($evenement->temps_fin))) }}
                            </p>
                        @else
                            <p><i
                                    class="fa-regular fa-calendar-check"></i>{{ str_replace('X', 'à', ucwords(utf8_encode(strftime('%A %d %B X %H:%M', strtotime($evenement->temps_debut))))) }}
                            </p>
                            <p><i
